...
### `android-app/` — 手机端 App（claimer v0.3.0，独立运行，无需电脑）
Kotlin 无障碍服务，零第三方依赖。核心机制：
- **真值校验**：以宿主界面"X时Y分"总时长计数器为准，广告接口返回值一律不信
- **兜底补检**：每轮主循环开头先读计数器，抓残留广告/上一轮间隙的延迟入账（PC 版 `[pre]` 同款）
- **残留广告领养**：广告开着不是异常（可能是晚开的正常广告），等它播完→点跳过→处理"确定要退出吗"确认框，绝不强杀丢弃奖励
- **事件过滤**：只从 `WINDOW_STATE_CHANGED` 取 Activity 类名；`CONTENT_CHANGED` 的 className 是控件名，会污染判定（v0.2.x 卡死根因）
- **计数拆分**：开不出任务页（库存真空）只退避不退出；连续 6 次"开了未到账"才重置加载器
- **自适应退避**：30→120s 指数退避 ±20% 抖动
- **网络判定**：`ConnectivityManager` `NET_CAPABILITY_VALIDATED`（应用沙箱无 ping 权限，EPERM），判定失败按有网处理
...
实测（v0.3.0）：残留广告领养 + 兜底补检连续 3 轮各到账 +20 分钟。
