...
do_ad() returns a STATUS string; patient_loop still verifies every reward
against the app's real 可暂停时长 (never trust the status blindly):
  "watched" - an ad stayed up long enough to plausibly pay out
  "flash"   - ad activity died in <FLASH_SEC; nothing played, cheap to verify
  "no_open" - no ad activity ever took focus (inventory / retry button)
...
from android_control import (adb, bring_front, deny_honor_jump, dump_texts,
                             ...
                             keep_awake, log, screencap_bytes, tap, use_device)
...
FOCUS_POLL = 0.5
FLASH_SEC = 3.0                     # ad gone faster than this = nothing played
...
def wait_ad_open(cfg, timeout=AD_OPEN_TIMEOUT):
...
def do_ad(i, cfg):
    """One ad attempt. Returns "watched" / "flash" / "no_open"."""
    # Ad inventory sometimes fails to load; the button then says
    # "广告加载失败，点击重试" and a tap opens nothing. Try twice, cheaply.
    for attempt in (1, 2):
        tap(*cfg.watch)
        f, a = wait_ad_open(cfg)
        if is_ad(f, a):
            break
        if attempt == 1:
            log(f"[{i}] no ad opened -> retap")
            time.sleep(1.5)
    else:
        log(f"[{i}] no ad opened (inventory / retry button)")
        return "no_open"
    ...
    # Unknown layout: the SDK UI is not dumpable, so we cannot read the
    # "领取成功" / countdown text to confirm the reward banked. The safe move is
    # to WAIT for playback to actually finish before closing - tapping skip
    # mid-video forfeits the reward (the old fixed 15s sleep did exactly that
    # on 20-30s videos, and never verified the reward landed).
    # Strategy: poll `focus()` (cheap). Only start tapping skip once past a
    # safe video length; if the ad activity ever leaves the top (playback
    # ended, reward banked) we stop waiting early. Hard cap so a stuck ad
    # can't hang the loop.
    UNKNOWN_MAX = 45      # hard cap (s) - a stuck ad must not hang the loop
    SAFE_VIDEO = 30       # rewarded videos are almost always <= 30s
    start = time.time()
    end = start + UNKNOWN_MAX
    last_tap = 0.0
    last_shot = b""
    last_shot_t = 0.0
    while time.time() < end:
        f, a = focus()
        if not is_ad(f, a):        # ad closed / end-card gone -> reward banked
            break
        el = time.time() - start
        if el >= SAFE_VIDEO and time.time() - last_tap >= 4:
            tap(*cfg.skip)
            last_tap = time.time()
        # Frozen-frame watchdog: on a bad network a video buffers forever and
        # blind waiting burns UNKNOWN_MAX every time. Two byte-identical frames
        # 8s apart (past the 15s grace period) mean stuck - bail and retry.
        if el >= 15 and time.time() - last_shot_t >= 8:
            shot = screencap_bytes()
            if shot and shot == last_shot:
                log(f"[{i}] frozen frame at {el:.0f}s -> ad stuck")
                break
            last_shot, last_shot_t = shot, time.time()
        time.sleep(FOCUS_POLL)
    el = time.time() - start
    f, a = focus()
    if is_ad(f, a):                # still up at cap -> one final close attempt
        tap(*cfg.skip)
        time.sleep(1.5)
    close_ad(i, cfg, f"unknown -> waited {el:.0f}s for playback")
    return "flash" if el < FLASH_SEC else "watched"
...
def main():
    ...
    serial = ensure_ready()
    if not serial:
        log("ERROR: no authorized device. Plug in the phone, turn on USB 调试, "
    ...
    for i in range(1, args.count + 1):
        if not ensure_main(cfg):
            log(f"[{i}] cannot reach main, reset")
            bring_front(cfg.main)
            time.sleep(2)
        try:
            status = do_ad(i, cfg)
        ...
        if status != "no_open":
            ...
            if noopen_streak >= cfg.max_noopen:
                log(f"STOP: {noopen_streak} ads in a row never opened "
                    f"(likely daily quota done or no ad inventory).")
                break
        time.sleep(0.8)
        log(f"    elapsed {time.time()-t0:.0f}s  watched={watched}")

    dur = time.time() - t0
    avg = dur / watched if watched else 0
    log(f"DONE watched={watched} in {dur:.0f}s ({avg:.1f}s/ad)")
    return 0
